(function () {
  const AEROLINEA = 'binter';
  const FILAS = 6;
  const COLUMNAS = 4; // visual COLUMNAS+1 con pasillo en posición 3
  const PRECIOS_CATEGORIAS = { business: 200, economica: 120, lowcost: 80 };
  ...
  function construirAsientos() {
    ...
    for (let r = 1; r <= FILAS; r++) {
      for (let c = 1; c <= COLUMNAS; c++) {
        asientos.push({ id: `R${r}C${c}`, fila: r, col: c, categoria: (r === 1 ? 'business' : (r < 4 ? 'economica' : 'lowcost')) });
      }
    }
    return asientos;
  }
  ...
  function dibujarTablaYControles() {
    ...
    html += '<label><input type="checkbox" id="residente_binter"> Soy residente (75% descuento)</label> ';
    html += '<span id="info_binter">Total: 0€ (0 asientos)</span> ';
    html += '<button id="confirmar_binter">Confirmar reserva (mantén pulsado)</button> ';
    html += '<button id="cancelar_binter">Cancelar selección</button>';
    html += '</div>';
    ...
    setTimeout(() => {
      ...
      celdas.forEach(td => {
        ...
        td.onclick = function () {
          const id = td.getAttribute('data-asiento');
          // si está vendido, permitir liberarlo mediante prompt
          if (obtenerVendidos().includes(id)) {
            ...
            if (respuesta && respuesta.trim().toUpperCase() === 'S') {
              ...
              const indice = vendidosAct.indexOf(id);
              if (indice !== -1) {
                vendidosAct.splice(indice, 1);
                ...
                alert(`Asiento ${id} puesto en libre.`);
              } else {
                alert('No se pudo liberar el asiento.');
              }
            }
            return;
          }
      ...
      const residente = document.getElementById('residente_binter');
      const info = document.getElementById('info_binter');
      const confirmar = document.getElementById('confirmar_binter');
      const cancelar = document.getElementById('cancelar_binter');
  ...
  try { dibujarTablaYControles(); } catch (e) { console.error('Error Binter:', e); }

  window.reservarPlazaBinter = reservarPlaza;
  window.liberarPlazaBinter = liberarPlaza;
})();